    ...
    <div>
      ...
      <section className="relative bg-slate-900 text-white">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-14 md:py-20">
          <div className="text-xs uppercase tracking-[0.3em] text-teal-300 font-semibold">{cat ? "Category" : q ? "Search results" : "Shop"}</div>
          <h1 className="font-display text-5xl md:text-6xl mt-3">{cat ? cat.name : q ? `"${q}"` : "All products"}</h1>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10">
        ...
        <div className="mb-6 flex gap-2 overflow-x-auto scroll-hide">
          <Link to="/shop" className={`whitespace-nowrap rounded-full px-4 py-2 text-sm border transition-colors ${!category ? "bg-slate-900 text-white border-slate-900" : "border-stone-200 hover:border-teal-700"}`} data-testid="filter-cat-all">All</Link>
          {CATEGORIES.map((c) => (
            <Link key={c.slug} to={`/shop/${c.slug}`} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm border transition-colors ${category === c.slug ? "bg-slate-900 text-white border-slate-900" : "border-stone-200 hover:border-teal-700"}`} data-testid={`filter-cat-${c.slug}`}>{c.name}</Link>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-9">
            <div className="flex items-center justify-between mb-5">
              <div className="text-sm text-slate-500"><span className="font-semibold text-slate-900">{products.length}</span> products</div>
            ...
            {products.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                <div className="font-display text-3xl text-slate-900">Nothing here yet.</div>
                <p className="mt-2">Try a different filter, or let our concierge source it.</p>
                <Link to="/find-it-for-me" className="mt-4 inline-block rounded-full bg-slate-900 text-white px-5 py-2 text-sm">Find-it-for-me</Link>
              </div>
            ) : (
      ...
      {filtersOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/40" onClick={() => setFiltersOpen(false)}>
          <div className="absolute bottom-0 left-0 right-0 rounded-t-3xl bg-white p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <FilterPanel filters={filters} setFilters={setFilters} />
            <button onClick={() => setFiltersOpen(false)} className="mt-6 w-full rounded-full bg-slate-900 text-white py-3 font-semibold" data-testid="apply-filters">View {products.length} products</button>
          </div>
        </div>
      )}
    </div>
...
function FilterPanel({ filters, setFilters }) {
  return (
    <div className="space-y-6">
      <div>
        <div className="text-xs uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Availability</div>
        <div className="space-y-2 text-sm">
          {[
            { v: "all", l: "All" },
            { v: "ready_stock", l: "Ready in Maldives" },
            { v: "preorder", l: "India Pre-order" },
            { v: "custom_sourcing", l: "Custom Sourcing" },
          ].map((o) => (
      ...
      <div>
        <div className="text-xs uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Max price</div>
        ...
        <div className="text-sm text-slate-700 mt-1">Up to MVR {filters.priceMax.toLocaleString()}</div>
      </div>
      <div className="md:hidden">
        <div className="text-xs uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Sort</div>
